import React, { useState, useEffect } from 'react';
import { useNavigate } from "react-router-dom";
import './Header.css';
import transformRole from '../hooks/transfromRole';
import useAuth from "../hooks/useAuth";
import userAvatar from '../logo.png';
import useLogout from '../hooks/useLogout';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faRightFromBracket, faBars, faTimes } from '@fortawesome/free-solid-svg-icons';

function Header({ isSidebarOpen, toggleSidebar }) {
    const { auth } = useAuth();

    const username = auth?.username || "User";
    const role = auth?.role
    const avatar = auth?.avatar
        ? `https://cdn.discordapp.com/avatars/${auth.discordId}/${auth.avatar}.png`
        : defaultAvatar;


    const navigate = useNavigate();
    const logout = useLogout();

    const userRole = transformRole(role);

    const signOut = async () => {
        await logout();
        navigate('/login');
    };

    const handleSignOut = () => {
        signOut();
    };

    return (
        <div className="header" style={{
            width: isSidebarOpen ? 'calc(100% - 220px)' : '100%',
            left: isSidebarOpen ? '220px' : '0'
        }}>
            <div className="toggle-button" onClick={toggleSidebar}>
                <FontAwesomeIcon icon={isSidebarOpen ? faTimes : faBars} />
            </div>
            <div className="user-info">
                <div className="header-user-avatar">
                    <img className="header-user-icon" src={avatar} alt="User Avatar" />
                </div>
                <div className="user-details">
                    <span className="username">{username}</span>
                    <span className="role">{userRole}</span>
                </div>
                <button className="Btn" onClick={handleSignOut}>
                    <div className="sign">
                        <svg viewBox="0 0 512 512">
                            <path d="M377.9 105.9L500.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L377.9 406.1c-6.4 6.4-15 9.9-24 9.9c-18.7 0-33.9-15.2-33.9-33.9l0-62.1-128 0c-17.7 0-32-14.3-32-32l0-64c0-17.7 14.3-32 32-32l128 0 0-62.1c0-18.7 15.2-33.9 33.9-33.9c9 0 17.6 3.6 24 9.9zM160 96L96 96c-17.7 0-32 14.3-32 32l0 256c0 17.7 14.3 32 32 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-64 0c-53 0-96-43-96-96L0 128C0 75 43 32 96 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32z"></path>
                        </svg>
                    </div>
                    <div className="text">Logout</div>
                </button>
            </div>
        </div>
    );
}

export default Header;
